...
get_signal = function(trait, tree, data){
    ...
    rep_l = sort(replicate(3, random_treelen(tree, n)))
...
writeLines("# QC Report for the phylogenetic tree", fileConn)
writeLines("", fileConn)
writeLines("## Comparison to previous iteration's tree", fileConn)
writeLines(paste("wRF distance between previous and new iteration's trees:", obs), fileConn)
writeLines(paste("This is equivalent to a number of random NNIs equal to roughly:", thresh), fileConn)
writeLines(paste("This is equivalent to a number of random NNIs equal to roughly:", thresh), fileConn)
writeLines("", fileConn)

writeLines("## Clustering of Pangolin lineages", fileConn)
writeLines(paste(sum(clustering$p<0.5), "out of", nrow(clustering), "Pangolin lineages show significant clustering"), fileConn)
writeLines("", fileConn)
writeLines("#### Summary of p values", fileConn)
close(fileConn)
